...
class SAHI:
    ...
    def slice_image(image: np.ndarray, slice_height: int, slice_width: int,
        ...
        if img_height % step_h != 0:
            for x in range(0, img_width - slice_width + 1, step_w):
                patch = image[img_height - slice_height:img_height, x:x + slice_width]
        ...
        if img_height % step_h != 0 and img_width % step_w != 0:
            patch = image[img_height - slice_height:img_height, img_width - slice_width:img_width]
        ...
        for pred, y, x in preds:
            if pred.numel() > 0:
                for detection in pred.cpu().numpy():
                    ...
                    all_detections.append([x1 + x, y1 + y, x2 + x, y2 + y, confidence, class_id])
    ...
    def run(self, image: np.ndarray, slice_height: int, slice_width: int,
            overlap_height_ratio: float, overlap_width_ratio: float):
        
        sliced_images = SAHI.slice_image(image, slice_height, slice_width, 
